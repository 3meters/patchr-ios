//
//  MostActiveTableViewController.swift
//  Patchr
//
...
class MostActiveTableViewController: QueryResultTableViewController {
    ...
    override func configureCell(cell: UITableViewCell, object: AnyObject) {
        if let queryResult = object as? QueryResult {
            if let patch = queryResult.entity_ as? Patch {
                ...
                if patch.count != nil {
                    cell.textLabel?.text = cell.textLabel?.text?.stringByAppendingString(" \(patch.count) messages")
                }
            } else {
