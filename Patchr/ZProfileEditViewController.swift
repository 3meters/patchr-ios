    ...
    *--------------------------------------------------------------------------------------------*/
    ...
    override func initialize() {
        ...
        if self.inputState == State.Onboarding {
            ...
            let doneButton   = UIBarButtonItem(title: "Join", style: UIBarButtonItemStyle.plain, target: self, action: #selector(ZProfileEditViewController.doneAction(sender:)))

            self.navigationItem.rightBarButtonItems = [doneButton]
            self.navigationItem.leftBarButtonItems = nil

            self.joinButton.addTarget(self, action: #selector(ZProfileEditViewController.doneAction(sender:)), for: .touchUpInside)
            self.termsButton.addTarget(self, action: #selector(ZProfileEditViewController.termsAction(sender:)), for: .touchUpInside)
        }
        else {
            ...
            /* Navigation bar buttons */
            let cancelButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.cancel, target: self, action: #selector(ZProfileEditViewController.cancelAction(sender:)))
            let deleteButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.trash, target: self, action: #selector(ZProfileEditViewController.deleteAction(sender:)))
            let doneButton   = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.save, target: self, action: #selector(ZProfileEditViewController.doneAction(sender:)))

            self.navigationItem.leftBarButtonItems = [cancelButton]
            self.navigationItem.rightBarButtonItems = [doneButton, Utils.spacer, deleteButton]

            self.changePasswordButton.addTarget(self, action: #selector(ZProfileEditViewController.changePasswordAction(sender:)), for: .touchUpInside)
        }
    }
    ...
    func post() {
        ...
        progress?.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(ZProfileEditViewController.progressWasCancelled(sender:))))
