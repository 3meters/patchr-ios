...
class CreateEditPatchViewController: UITableViewController, UITableViewDataSource, UITableViewDelegate {
	...
	private let DefaultTypeRow  = 2
	// "Place"
	private let ButtonSection   = 4
	private let CreateButtonRow = 0
	private let DeleteButtonRow = 1
	// in initial table view. Won't necessarily be at this index after viewDidLoad
	...
	private var isEditingPatch: Bool {
		return patch != nil
	}
	...
	var patchName: String {
		get {
			return patchNameField.text ?? ""
		}
		set {
			patchNameField.text = newValue
		}
	}

	var patchDescription: String? {
		get {
			return patchDescriptionField.text
		}
		set {
			patchDescriptionField.text = newValue
		}
	}

	var patchImage: UIImage {
		get {
			return patchImageView.image ?? UIImage(named: "Placeholder patch header")!
		}
		set {
			patchImageView.image = newValue
		}
	}
	...
	var patchType: String {
		get {
			for row in 0 ..< patchTypeIDs.count {
				let cell
				= tableView.cellForRowAtIndexPath(NSIndexPath(forRow: row, inSection: TypeSection))
				if let theCell = cell {
	...
	override func viewWillAppear(animated: Bool) {
		...
		observerObject = TextFieldChangeObserver(patchNameField) {
			[unowned self] in
	...
	func createPatchButtonAction(sender: AnyObject) {
		...
		println(parameters)
		proxibase.createObject("data/patches", parameters: parameters) {
			response, error in
	...
	func updatePatch() {
		...
		proxibase.updateObject("data/patches/\(patch!.id_)", parameters: parameters) {
			response, error in
			self.processing = false
			progress.hide(true, afterDelay: 1.0)
			dispatch_async(dispatch_get_main_queue()) {
	...
	func deletePatchButtonAction(sender: AnyObject) {
		self.ActionConfirmationAlert(LocalizedString("Confirm Delete"), message: LocalizedString("Are you sure you want to delete this?"), actionTitle: LocalizedString("Delete"), cancelTitle: LocalizedString("Cancel")) {
			...
			ProxibaseClient.sharedInstance.deleteObject(patchPath) {
				response, error in
