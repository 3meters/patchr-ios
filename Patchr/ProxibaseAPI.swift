		...
		self.fetchAllCategories() {
			...
			if let serverError = ServerError(error) {
				...
				dispatch_async(dispatch_get_main_queue())
				{
					...
					serverResponse.forEachResultObject {
						resultObject in
...
//
	...
	public func uploadFileToS3(fileURL: NSURL, contentType: String, bucket: String, key: String, completion: S3UploadCompletionBlock) {
		...
		let credProvider
						  = AWSStaticCredentialsProvider(accessKey: PatchrS3Key, secretKey: PatchrS3Secret)
	...
	public func signIn(email: NSString, password: NSString, completion: ProxibaseCompletionBlock) {
		let parameters
		= ["email": email, "password": password, "installId": self.installationIdentifier]
	...
	public func createUser(name: String, email: String, password: String, parameters: NSDictionary? = nil, completion: ProxibaseCompletionBlock) {
		...
		self.performPOSTRequestFor("user/create", parameters: createParameters) {
			response, error in
	...
	func queuePhotoUploadInParameters(parameters: NSMutableDictionary, queue: dispatch_queue_t, bucket: S3Bucket) {
		...
		if let userId = self.userId {
			if let photo = parameters["photo"] as? UIImage {
				...
				dispatch_async(queue) {
					self.uploadImageToS3(photo, bucket: self.bucketInfo[bucket]!.name, key: profilePhotoKey) {
						result, error in
	...
	public func deleteLink(linkID: String, completion: ProxibaseCompletionBlock? = nil) {
		...
		self.performDELETERequestFor(linkPath, parameters: NSDictionary()) {
			response, error in
	...
	public func createLink(fromID: String, toID: String, linkType: LinkType, completion: ProxibaseCompletionBlock) {
		...
		self.performPOSTRequestFor("data/links", parameters: postParameters) {
			response, error in
			completion(response: response, error: error)
		}
	}
	...
	public func findLink(fromID: String, toID: String, linkType: LinkType, completion: ProxibaseCompletionBlock) {
		...
		= ["query": ["_from": fromID, "_to": toID, "type": linkType.rawValue]]
	...
	public func fetchMessagesForPatch(patchId: String, limit: NSInteger = 50, skip: NSInteger = 0, links: [Link] = [], completion: (response:AnyObject?, error:NSError?) -> Void) {
		let allLinks
		= [Link(from: .Messages, type: .Content, linkFields: "_id")] + links + standardPatchLinks()
		let parameters = [
	...
	public func fetchInterestingPatches(location: CLLocationCoordinate2D, limit: NSInteger = 50, skip: NSInteger = 0, completion: (response:AnyObject?, error:NSError?) -> Void) {
		...
		let parameters: Dictionary<String, AnyObject> = [
				...
				"linked": allLinks.map {
					$0.toDictionary()
				} // Doesn't work the same as /find API
		]
	...
	private func authenticatedParameters(var parameters: NSDictionary) -> NSDictionary {
		if self.authenticated {
			var authParameters
			= NSMutableDictionary(dictionary: ["user": self.userId!, "session": self.sessionKey!])
...
public class Link {
	...
	func toDictionary() -> Dictionary<String, AnyObject> {
		...
		if subLinks != nil {
			dictionary["linked"] = self.subLinks!.map {
				$0.toDictionary()
			}
		}
